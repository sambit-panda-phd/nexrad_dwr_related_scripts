# -*- coding: utf-8 -*-
"""
Created on Wed Dec 11 14:25:18 2019
...
clb = np.loadtxt('./new_common_labels/new_common_labels_20170826.txt')
nlb = np.loadtxt('./new_counts/new_nexrad_hid_counts_20170826.txt')
slb = np.loadtxt('./new_counts/new_sac_hid_counts_20170826.txt')
time = np.loadtxt('./new_time_list.txt')
...
for i in range(0,nlb.shape[0],5):
    ...
    plt.savefig('./new_count_plots/%s_counts.png' %(str(int(time[i]))), dpi=600)
    plt.clf()
    plt.close()
